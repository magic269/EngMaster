...
// دالة التحقق من محاولات تسجيل الدخول
...
// دالة زيادة محاولات تسجيل الدخول
function incrementLoginAttempts() {
  const attempts = parseInt(localStorage.getItem('loginAttempts') || '0') + 1;
  localStorage.setItem('loginAttempts', attempts);
}

// دالة إعادة تعيين محاولات تسجيل الدخول
...
async function handleLogin(e) {
  ...
  try {
    ...
    const password = formData.get('password'); // لا تعقيم لكلمة المرور لأنها تُرسل مباشرة إلى Firebase
...
async function handleRegister(e) {
  ...
  try {
    ...
    const errorMsg = error.code === 'auth/email-already-in-use' ? 'البريد الإلكتروني مستخدم بالفعل!' :
                     ...
                     error.code === 'auth/weak-password' ? 'ك MATRIX المرور ضعيفة جدًا!' :
                     error.message;
...
function setupEventListeners() {
  ...
  // منع اللصق في حقول كلمة المرور
  document.querySelectorAll('input[type="password"]').forEach((input) => {
    input.addEventListener('paste', (e) => e.preventDefault());
  });

  // تعقيم المدخلات في الوقت الحقيقي
